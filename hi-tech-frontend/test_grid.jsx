...
const GridPropertyTile = ({ property, onOpen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-black px-5 py-4 translate-y-full transition-transform duration-300 ease-out group-hover:translate-y-0 flex items-center justify-between">
...
const FeaturedListings = ({ properties = [], loading, onOpen, setCurrentPage }) => {
  ...
  // Take exactly 5 properties for the perfect 3x2 grid (1 text block + 5 images = 6 cells)
    ...
    <section className="bg-white">
      ...
      <div className="w-full px-2 sm:px-4 lg:px-8 py-10 sm:py-16 max-w-[120rem] mx-auto">
        ...
        {loading ? (
          ...
          <p className="border border-dashed border-black/10 py-24 text-center text-sm text-matte/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[10px] md:gap-[15px] lg:gap-[20px]">
            ...
            <Reveal variant="left" duration={800} className="h-full">
              <div className="bg-white flex flex-col justify-center items-center text-center h-full min-h-[320px] md:min-h-[350px] lg:min-h-[380px] p-6 relative overflow-hidden">
                ...
                <h2 className="text-[40px] sm:text-[50px] lg:text-[55px] font-black text-[#111] uppercase tracking-tighter font-poppins leading-[1]">
                  Featured<br/>
                  <span 
                    className="block text-transparent uppercase tracking-wider text-[34px] sm:text-[42px] lg:text-[48px] mt-2 font-semibold"
                ...
                <div className="mt-14 flex items-center relative group cursor-pointer" onClick={goToListings}>
                  {/* Horizontal Line cutting into button */}
                  <div className="h-[1px] w-12 sm:w-16 bg-black mr-[-10px] z-10 relative transition-all duration-300 group-hover:bg-[#d3a950]"></div>
                  <button
                    className="pl-14 pr-6 py-[14px] border border-black text-black text-[12px] font-medium uppercase tracking-[0.15em] transition-colors duration-300 relative bg-white group-hover:bg-black group-hover:text-white group-hover:border-black"
                  >
                    View All Properties +
                  </button>
                </div>
            ...
            {/* Fill remaining cells if < 5 properties */}
            {Array.from({ length: 5 - displayProperties.length }).map((_, idx) => (
               <div key={empty-} className="bg-white h-[320px] sm:h-[350px] lg:h-[380px]"></div>
            ))}
          </div>
        )}
      </div>
