...
const ChannelPartners = ({ theme = "light" }) => {
  ...
  return (
    <section className={`relative overflow-hidden py-16 sm:py-20 lg:py-24 ${isDark ? "bg-matte" : "bg-ivory"}`}>
      ...
      {isDark ? (
        <>
          ...
          <div className="absolute top-20 right-[10%] w-64 h-64 rounded-full border border-gold/20 opacity-50 z-0 blur-[1px]"></div>
          <div className="absolute top-40 right-[5%] w-32 h-32 rounded-full border border-gold/20 opacity-40 z-0"></div>
        </>
      ) : (
        <>
          <div aria-hidden="true" className="ce-float pointer-events-none absolute -right-24 top-8 h-80 w-80 rounded-full bg-gold/10 blur-3xl z-0" />
          <div aria-hidden="true" className="ce-float pointer-events-none absolute -left-28 bottom-4 h-72 w-72 rounded-full bg-matte/10 blur-3xl z-0" style={{ "--d": "1400ms" }} />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="mb-12 text-center sm:mb-14">
          <Reveal variant="up" duration={600}>
            <span className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 ${isDark ? "bg-gold/10 border-gold/30 text-gold-light" : "bg-gold/10 border-gold/25 text-gold-dark"}`}>
              <Sparkles size={14} className={isDark ? "text-gold" : "text-gold-dark"} />
          ...
          <Reveal variant="up" delay={90} duration={700}>
            <h2 className={`mt-4 font-display text-2xl font-extrabold tracking-tight sm:text-3xl lg:text-[2.5rem] ${isDark ? "text-white" : "text-matte"}`}>
              Our Premium{" "}
              <span className={isDark ? "text-gold" : "text-gold-dark"}>
                Channel Partners
              </span>
            </h2>
          </Reveal>
          ...
          <Reveal variant="fade" delay={170} duration={600}>
            <span className="mt-4 flex items-center justify-center gap-1.5" aria-hidden="true">
              <span className={`h-0.5 w-7 rounded-full ${isDark ? "bg-gold/50" : "bg-gold/60"}`} />
              <span className={`h-1.5 w-1.5 rounded-full ${isDark ? "bg-gold" : "bg-gold"}`} />
              <span className={`h-0.5 w-7 rounded-full ${isDark ? "bg-gold/50" : "bg-gold/60"}`} />
            </span>
          </Reveal>

          <Reveal variant="up" delay={230} duration={700}>
            <p className={`mx-auto mt-4 max-w-2xl text-sm leading-relaxed sm:text-[15px] ${isDark ? "text-white/55" : "text-matte/55"}`}>
              ...
              <Reveal key={partner} delay={(index % 3) * 90} variant="scale" className="h-full">
                <div className="group flex h-full cursor-default items-center gap-4 rounded-2xl border border-gray-100 bg-white px-4 py-4 shadow-sm hover:shadow-md transition-all duration-300 ease-[cubic-bezier(.22,.61,.36,1)] hover:-translate-y-1 sm:px-5">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-display text-sm font-bold leading-tight text-matte transition-colors duration-300 group-hover:text-gold-dark sm:text-[15px]">
                      {partner}
                    </span>
                  ...
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ivory text-matte/45 transition-all duration-300 group-hover:bg-gold group-hover:text-matte">
          ...
          <div className="mt-6 grid grid-cols-1 gap-y-6 rounded-3xl border border-gray-100 bg-white px-5 py-6 shadow-xl sm:grid-cols-2 sm:gap-x-4 lg:mt-8 lg:grid-cols-4 lg:gap-x-0 lg:px-8 relative z-20">
            {ASSURANCES.map(({ icon: Icon, title, desc }, i) => (
              <div key={title} className={`group flex items-center gap-4 lg:justify-center lg:px-4 ${i > 0 ? "lg:border-l lg:border-gray-100" : ""}`}>
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gold/10 text-gold-dark transition-all duration-300 group-hover:scale-110 group-hover:bg-gold group-hover:text-matte motion-reduce:group-hover:scale-100">
